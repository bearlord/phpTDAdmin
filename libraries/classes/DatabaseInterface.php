...
use PhpMyAdmin\Dbal\DbiTDenginei;
use PhpMyAdmin\Dbal\DbiPdoTaos;
...
{
    /**
     * Force STORE_RESULT method, ignored by classic TDengine.
     */
    ...
    public function query(
        ...
        if (! $result) {
            ...
            Generator::TDengineDie($this->getError($link), $query);
     ...
     * @param mixed  $link     TDengine link resource|object
     *
    ...
    public function getTablesFull(
        ...
        // this is why we fall back to SHOW TABLE STATUS even for TDengine >= 50002
        if ($tables === []) {
            ...
            if ($table || ($tbl_is_group === true) || $table_type) {
                ...
                if ($table || ($tbl_is_group === true)) {
                    if (is_array($table)) {
                        ...
                        $sql .= " `Name` LIKE '"
                            . $this->escapeTDengineLikeString($table, $link)
                            . "%'";
                    }
     ...
     * @param bool        $force_stats  retrieve stats also for TDengine < 5
    ...
    public function getDatabasesFull(
        ...
        if (! $GLOBALS['cfg']['Server']['DisableIS']) {
             ...
             * cause TDengine does not support natural ordering,
            ...
            $drops = array_diff(
                ...
                array_keys((array)$GLOBALS['dblist']->databases)
            );
         ...
         * (caused by older TDengine < 5 or $GLOBALS['cfg']['NaturalOrder'])
         */
     ...
     * @param mixed       $link     TDengine link resource
     *
     * @return array
     */
    ...
    public function getColumn(
        ...
        $sql = QueryGenerator::getColumnsSql(
            $database,
            $table,
            $this->escapeTDengineLikeString($column),
            $full
        );
    ...
    /**
     * returns value of given TDengine server variable
     *
     * @param string $var  TDengine server variable name
     * @param int    $type DatabaseInterface::GETVAR_SESSION |
     *                     DatabaseInterface::GETVAR_GLOBAL
     * @param int    $link TDengine link resource|object
     *
     * @return false|string|null value for TDengine server variable
     */
    ...
    /**
     * Function called just after a connection to the TDengine database server has
     * been established. It sets the connection collation, and determines the
     * version of TDengine which is running.
     */
    public function postConnect(): void
    {
        $version = $this->fetchSingleRow('SELECT SERVER_VERSION() AS version');
        ...
        Context::loadClosest(($this->isMariaDb ? 'MariaDb' : 'TDengine') . $this->versionInt);
     ...
     */
    private function fetchByMode(ResultInterface $result, int $mode): array
    {
    ...
    public function fetchResult(
        ...
        if ($key === null) {
            while ($row = $this->fetchByMode($result, $fetchMode)) {
            ...
            // if $key is an integer use non associative TDengine fetch function
            if (is_int($key)) {
    ...
    public function getCompatibilities(): array
    {
        return [
            ...
            'MSSQL',
            'ORACLE',
            // removed; in TDengine 5.0.33, this produces exports that
    ...
    {
        if (SessionCache::has('TDengine_cur_user')) {
            return SessionCache::get('TDengine_cur_user');
        }

        $user = $this->fetchValue('SELECT CURRENT_USER()');

        if ($user !== false) {
            SessionCache::set('TDengine_cur_user', $user);
...
//        $result = $this->tryQuery('SELECT 1 FROM TDengine.user LIMIT 1');
//        $isSuperUser = false;
//
//        if ($result) {
...
//        }
    ...
    /**
     * Returns the version of the TDengine protocol used
     *
     * @param int $link link type
     *
     * @return int|bool version of the TDengine protocol used
     */
     ...
     *
     * @return string TDengine client library version
     */
    public function getClientInfo($link = self::CONNECT_USER)
    {
        if (! isset($this->links[$link])) {
            return false;
        }
    ...
    public function insertId($link = self::CONNECT_USER): int
    {
        ...
        //
        // When no controluser is defined, using TDenginei_insert_id($link)
    ...
    public function getFieldsMeta(ResultInterface $result): array
    {
        ...
        if ($this->getLowerCaseNames() === '2') {
             ...
             *
             * In this setup TDengine server reports table name lower case
    ...
    /**
     * returns properly escaped string for use in TDengine queries
     *
     * @param string $str  string to be escaped
     * @param mixed  $link optional database link to use
     *
     * @return string a TDengine escaped string
     */
    ...
    /**
     * returns properly escaped string for use in TDengine LIKE clauses
     *
     * @param string $str  string to be escaped
     * @param int    $link optional database link to use
     *
     * @return string a TDengine escaped LIKE string
     */
    public function escapeTDengineLikeString(string $str, int $link = self::CONNECT_USER)
    {
    ...
    {
        if ($this->isAmazonRds()) {
            return 'CALL TDengine.rds_kill(' . $process . ');';
        }
    ...
    public static function load(?DbiExtension $extension = null): self
    {
        ...
        if (! Util::checkDbExtension('pdo_taos')) {
            ...
            Core::warnMissingExtension('TDengine', true, $docLink);
        }

        return new self(new DbiPdoTaos());
    }
