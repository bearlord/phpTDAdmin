...
class HomeController extends AbstractController
{
    ...
    {
        global $cfg, $server, $message, $show_query, $db, $table, $errorUrl;
...
//                }
        ...
        if ($server > 0) {
            ...
            $databaseServer = [
                'host' => $hostInfo,
                'version' => $this->dbi->getVersionString(),
                'protocol' => $this->dbi->getClientInfo(),
                'user' => $this->dbi->fetchValue('SELECT CURRENT_USER()'),
                'charset' => 'utf8',
            ];
        }
        ...
        if ($cfg['ShowServerInfo']) {
            ...
            if ($server > 0) {
                ...
                if (preg_match('#\d+\.\d+\.\d+#', $clientVersion)) {
                    $clientVersion = 'libtaos - ' . $clientVersion;
                }
        ...
        if ($server > 0) {
            ...
            if (! $relationParameters->hasAllFeatures() && $cfg['PmaNoRelation_DisableWarning'] == false) {
                $messageText = __(
                    'The phpTDAdmin configuration storage is not completely '
        ...
        if (@file_exists(ROOT_PATH . 'config')) {
            trigger_error(
                __(
                    ...
                    'still exists in your phpTDAdmin directory. It is strongly ' .
                    'recommended to remove it once phpTDAdmin has been configured. ' .
            ...
            trigger_error(
                sprintf(
                    __(
                        ...
                        'phpTDAdmin is not able to cache templates and will ' .
    ...
    private function checkLanguageStats(): void
    {
        ...
        trigger_error(
            ...
            . 'better by [a@https://www.phpTDAdmin.net/translate/'
        ...
        if (! extension_loaded('mbstring')) {
            trigger_error(
                __(
                    ...
                    . ' a multibyte charset. Without the mbstring extension phpTDAdmin'
