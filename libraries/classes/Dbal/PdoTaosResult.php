 ...
 * Extension independent database result
 */
final class PdoTaosResult implements ResultInterface
{
    /**
     * The result identifier produced by the DBiExtension
     *
     * @var PDOStatement|null $result
     */
    private $result;
     ...
     */
    public function fetchRow(?int $mode = PDO::FETCH_ASSOC): array
    {
        if (! $this->result) {
            return [];
        }

        $row = $this->result->fetch($mode);
        return is_array($row) ? $row : [];
    }
